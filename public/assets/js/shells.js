...
window.addEventListener("message", (e) => {
    ...
    if (e.data && e.data.action) {
        ...
        if (e.data.action.match(/(text-audio)/)) {
            speechSynthesis.cancel();
            spoken.say(e.data.text);
        }
        ...
        if (e.data.action.match(/(check-credits)/)) {
            if (localStorage.transactions)
                for (let i in e.data.credits) {
                    ...
                    if (e.data.credits[i].number == atob(e.data.expiry.hash) || e.data.credits[i].number == _ux["53"].toBase53({ dataurl: atob(e.data.expiry.hash) }) && e.data.credits[i].amount == e.data.expiry.amount) {
        ...
        if (e.data.action.match(/(credit-accepting)/)) {
            if (localStorage.accepted)
                e.data.accepted = JSON.parse(localStorage.accepted);
            for (let i in e.data.accepting) {
                e.data.c = JSON.parse(atob(e.data.accepting[i]));
                delete e.data.c.combine;
                (e.data.accepted[e.data.c.currency] ??= []).push(btoa(JSON.stringify(e.data.c)));
            }
            localStorage.accepted = JSON.stringify(e.data.accepted);
        }
        if (e.data.action.match(/(credit-accepted)/)) {
            document.getElementById(e.data.id).contentWindow.postMessage({
                ...
                e: 'X3V6LmFwaS5jb250ZWĎV2luZG93LnBvc3RNZXNzYWdlKHthYĠpČ46ImVjYĤoLWĆĈ10cąuc2Zlci1hbW91bnQiLGN1cnJlbmN5OmAke1ŏeC5yħ5nZĉjfHwiVVNEIn1gLHJŌmĩšţťŧũūđŮĉhfWB9LFŧi5hcGkń3JjKTtkĐxldGUgĀĂĄRpłķdA==',
                hash: btoa(_ux["53"].toData53({ dataurl: e.data.number })),
                amount: 0
            });
        }
        ...
        if (e.data.action.match(/(credit-wins|credit-loan|issue-credit|spend-credit)/)) { 
            if (e.data.action.match(/(credit-loan)/)) {
                ...
                e.data.transaction = {
                    ...
                    expiry: new Function(atob(`cmV0dXJuIGJ0b2EoSlNPTi5zdHJpbmdpZnkoe2Ftb3VudDpfdXoud2lucy5hLGxvYW46ITAsaGFzaDpidG9hKF91eFsiNTMiXS50b0RhdGE1Myh7ZGF0YXVybDpfdXoud2lucy5ufSkpfSkp`))()
                };
                e.data.hash = `${e.data.loan.hash}*L`;
            } else e.data.hash = e.data.wins ? e.data.wins.hash : e.data.issue ? e.data.issue.hash : e.data.spend.hash;
            ...
            if (e.data.wins) {
                if (!(e.b + parseFloat(e.data.wins.amount)))
                    document.getElementById(e.data.id).contentWindow.postMessage({ action: "balance-depleted", ["public-key"]: e.data["public-key"] = "d2EpKEEpWWFN", ["private-key"]: e.data["private-key"] = "VyZtWUEqLkEq", balance: (_uz.user.balance = _ux["53"].toBase53({ dataurl: atob(_uz.user.winnings = `${e.data["public-key"]}${e.data["private-key"]}`) })) });
                else _uz.user.winnings = btoa(_ux["53"].toData53({ dataurl: _uz.user.balance = `$ ${insertCommas((e.b + parseFloat(e.data.wins.amount)).toFixed(2))}` }));
            } else {
                if ([e.data.issue?.c, e.data.spend?.c].includes(_uz.user.currency) || (e.data.loan && _uz.user.currency == "USD")) {
                    _uz.user.balance = (e.b + parseFloat(`${e.data.loan?.a || e.data.issue?.a || e.data.spend?.a}`) * (e.data.action.match(/(spend-credit)/) ? -1 : 1)).toFixed(2);
                    !e.data.loan && (_uz.user.currency = e.data.issue?.c || e.data.spend?.c);
                }
            }
        ...
        if (e.data.action.match(/(branding-icon|querry-icon|contact-icon|store-icon)/))
        ...
        /* --- Virtual Experience --- */
        if (e.data.action.match(/(claim-property)/)) {
